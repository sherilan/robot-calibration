...
class Estimator(torch.nn.Module):
    ...
    def __init__(self, x0=0, y0=0, z0=0, a0=0, px0=0., py0=0., pz0=0.2):
        super().__init__()
        ...
        self.px = torch.nn.Parameter(torch.tensor(px0).double())
        self.py = torch.nn.Parameter(torch.tensor(py0).double())
        self.pz = torch.nn.Parameter(torch.tensor(pz0).double())
    ...
    def param_errors(self, *, x, y, z, a, px, py, pz):
        with torch.no_grad():
            return dict(
                x=(self.x - x).item(),
                y=(self.y - y).item(),
